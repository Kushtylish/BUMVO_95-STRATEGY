...
============================================
Research-based robust optimisation using bootstrap percentiles.
Implements: TSMOM + MA crossover + Keltner channels + Kelly sizing.
Includes full backtest, Monte Carlo drawdown, and equity curve plotting.
"""
...
# =============================================================================
# CONFIGURATION – ADAPT THESE PATHS / PARAMETERS
# =============================================================================
CSV_PATH = r"C:\Users\Kusht\Downloads\XAU_USD Historical Data (2)"   # <-- your file
INITIAL_CAPITAL = 10000.0
SPREAD = 0.002                # 0.2% transaction cost per trade (one‑way)

# Strategy hyperparameter grids
L_GRID = [20, 60, 180, 252]
POS_METHODS = ['fixed', 'kelly']

# Bootstrap settings
N_BOOTSTRAP = 500
BLOCK_SIZE = 10                # mean block length for stationary bootstrap

# =============================================================================
# DATA LOADER – Robust to commas, auto‑detects date & price columns
# =============================================================================
def load_xauusd_csv(filepath):
    """Load XAUUSD daily prices from CSV. Returns Series with DatetimeIndex."""
    ...
    # Clean price column (remove commas, spaces)
...
print(f"✅ Loaded {len(prices)} days: {prices.index[0]} to {prices.index[-1]}")
...
def tsmom_signal(close, L):
    """Time‑series momentum: sign of log return over last L days."""
    ret = np.log(close / close.shift(L))
    return np.sign(ret)

def ma_signal(close, fast=50, slow=200):
    """Moving average crossover: sign of (fast EMA - slow EMA)."""
    ema_fast = close.ewm(span=fast, adjust=False).mean()
    ema_slow = close.ewm(span=slow, adjust=False).mean()
    return np.sign(ema_fast - ema_slow)

def keltner_signal(close, period=20, mult=2.0):
    """Keltner channel breakout: +1 above upper band, -1 below lower band."""
...
def combined_signal(close, L, ma_fast=50, ma_slow=200, keltner_period=20, keltner_mult=2.0):
    """Hybrid signal: if any indicator is active, take its sign; else 0."""
...
def kelly_fraction(returns, window=252, rf=0.0):
    """Kelly fraction f* = (μ - rf) / σ², clipped to [-1,1]."""
...
# =============================================================================
# STRATEGY SIMULATION – Returns PnL as a Series aligned with returns index
# =============================================================================
def simulate_strategy(returns, prices, L, pos_sizing='fixed',
                      ma_fast=50, ma_slow=200,
                      keltner_period=20, keltner_mult=2.0,
                      spread=SPREAD, verbose=False):
    """
    returns : Series of daily returns (DatetimeIndex)
    prices  : Series of daily prices (DatetimeIndex, length = len(returns)+1)
    Returns dict with 'pnl' (Series), 'sharpe', 'return', 'max_dd'.
    """
    if isinstance(prices, pd.DataFrame):
        prices = prices.iloc[:, 0]

    # Generate signals (based on prices)
    ...
    if pos_sizing == 'fixed':
        ...
        position = signal * kelly
    else:
        raise ValueError("pos_sizing must be 'fixed' or 'kelly'")

    # Shift to next day (trade at next open)
    position = position.shift(1).fillna(0)
    # Align with returns index (returns is shorter)
    position = position.reindex(returns.index, method=None)

    # Daily PnL (fraction of capital)
    pnl = position * returns

    # Transaction costs – charged on absolute change in position
    ...
    for L in L_grid:
        for method in pos_sizing_methods:
            ...
            for boot_ret in tqdm(boot_returns_list, desc=f'L={L}, {method}', leave=False):
                ...
                # Reconstruct the price series from bootstrapped returns
                start_price = prices.iloc[0]
                boot_price = start_price * np.exp(np.cumsum(boot_ret))
                # Use integer index (no date assumptions inside bootstrap)
...
# =============================================================================
# META‑OPTIMISATION: CHOOSE BEST PERCENTILE (60‑95) USING VALIDATION SET
# =============================================================================
def select_percentile(returns, prices, L_grid, pos_sizing_methods,
    ...
    """Walk‑forward validation to choose the best percentile."""
    split_idx = int(len(returns) * train_ratio)
    train_ret = returns.iloc[:split_idx]
    test_ret  = returns.iloc[split_idx:]
    train_price = prices.iloc[:split_idx+1]   # need one extra for first return
    ...
    for pct in percentile_range:
        print(f"\n📊 Evaluating percentile = {pct}")
        best_L, best_method, _ = bootstrap_optimise(
            train_ret, train_price, L_grid, pos_sizing_methods,
            n_bootstrap=n_bootstrap, block_size=block_size, percentile=pct, verbose=True
        )
        # Evaluate on test set
        perf = simulate_strategy(test_ret, test_price, best_L, pos_sizing=best_method, verbose=False)
        ...
        print(f"   → Test Sharpe: {test_sharpe:.3f} (L={best_L}, {best_method})")
        if test_sharpe > best_test_sharpe:
            best_test_sharpe = test_sharpe
            best_percentile = pct

    print("\n--- Percentile selection results ---")
    for r in results_percentile:
        print(f"Pct={r['percentile']:2d}, L={r['best_L']:3d}, method={r['best_method']:6s}, test Sharpe={r['test_sharpe']:.3f}")
    print(f"\n✅ Selected percentile: {best_percentile} (test Sharpe = {best_test_sharpe:.3f})")
    return best_percentile, results_percentile
...
# =============================================================================
def main():
    # ----- Step 1: Meta‑optimisation of percentile -----
    best_pct, _ = select_percentile(
        returns, prices, L_GRID, POS_METHODS,
        n_bootstrap=200,          # faster for meta
        block_size=BLOCK_SIZE,
        train_ratio=0.8,
        percentile_range=range(60,96,5)
    )
